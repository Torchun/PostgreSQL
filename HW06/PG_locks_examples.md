...
-- Session #1
-- SELECT pg_backend_pid();
SELECT locktype, relation::REGCLASS, virtualxid AS virtxid, transactionid AS xid, mode, granted FROM pg_locks WHERE pid = 235;
...
UPDATE accounts SET amount = amount + 100.00 WHERE acc_no = 1;
SELECT * FROM locks_v WHERE pid = 315;
...
-- Session #1
SELECT * FROM locks_v WHERE pid = 241;
...
-- Session #1
SELECT * FROM locks_v WHERE pid = 255;
...
-- Session #1
SELECT * FROM locks_v WHERE pid = 380;
