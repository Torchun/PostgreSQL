...
drop schema no_part cascade;
drop schema by_hash cascade;
drop schema by_list cascade;
drop schema by_range cascade;
...
explain analyze
select * from by_hash.orders where order_id = 10000 order by order_id; -- Execution Time: 0.078 ms

explain analyze
select * from no_part.orders where order_id = 10000 order by order_id; -- Execution Time: 0.103 ms
...
explain analyze
select * from by_list.orders where order_id in (10, 20, 1) order by order_id; -- Execution Time: 0.338 ms
...
explain analyze
select * from by_range.orders where order_date < date'2020-02-01';



-- А если не будут созданы все секции. Например MODULUS 3, а будут созданы  таблица-секции REMAINED 0 и REMAINED 1, а REMAINED 2 не будет. Что будет тогда при вставке данных?
...
create table test_0 partition of test for values with (modulus 3, remainder 0);
create table test_1 partition of test for values with (modulus 3, remainder 1);
create table test_2 partition of test for values with (modulus 3, remainder 2);

insert into test(id, name) select 1, 'fio' from generate_series(1, 100000);
